...
- **CLI Tool**: `ralph` command installed to `/usr/local/bin/ralph` with `--max-iterations` option
...
4. AI-specific config: `ai/opencode/opencode.json` copied to `$HOME/.config/opencode/opencode.json`, and `ai/opencode/skills/` copied to `$HOME/.config/opencode/skills/` (Makefile will handle this).
