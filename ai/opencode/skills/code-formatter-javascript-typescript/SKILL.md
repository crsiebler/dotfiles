---
name: code-formatter-javascript-typescript
description: Formats JavaScript/TypeScript code with Prettier, lints with ESLint CLI, and validates Bun and Vitest workflows
---
...
6. Important: **do not use `bun test`** when the repository uses Vitest. It runs Bun's test runner, not Vitest.
7. On commit, Husky may run lint-staged to format and lint staged files:
...
- Ensure all code passes Prettier, ESLint CLI, Vitest, and pre-commit checks before commit or PR
- Repository Prettier and ESLint configuration is authoritative
