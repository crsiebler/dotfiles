"""Bounded file and value handling for this independently packaged XLSX helper."""
import io
import math
...
def inspect_package(content):
    if len(content) > MAX_FILE:
        raise DocumentError(2, 'input: XLSX exceeds 16 MiB')
    try:
        with zipfile.ZipFile(io.BytesIO(content)) as archive:
            members = archive.infolist()
            if len(members) > 2000 or sum(m.file_size for m in members) > 64 * 1024 * 1024:
                raise DocumentError(2, 'input: expanded XLSX limits exceeded')
            ...
            if any(m.flag_bits & 1 for m in members) or archive.testzip() is not None:
                raise DocumentError(2, 'input: invalid or encrypted XLSX package')
            if 'xl/workbook.xml' not in archive.namelist():
                raise DocumentError(2, 'input: expected a XLSX package')
    except zipfile.BadZipFile:
        raise DocumentError(2, 'input: invalid XLSX ZIP package') from None
