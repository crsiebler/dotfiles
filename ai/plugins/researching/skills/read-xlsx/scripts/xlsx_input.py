"""Read-only, bounded project input for the independent XLSX reader."""
import io
...
def load_source(root,value):
    ...
    if source.stat().st_size > 16*1024*1024:
        raise ReadError(2,'input: XLSX exceeds 16 MiB')
    content=source.read_bytes()
    try:
        with zipfile.ZipFile(io.BytesIO(content)) as archive:
            members=archive.infolist()
            if len(members)>2000 or sum(m.file_size for m in members)>64*1024*1024:
                raise ReadError(2,'input: expanded XLSX limits exceeded')
            ...
            if 'xl/workbook.xml' not in archive.namelist():
                raise ReadError(2,'input: expected XLSX package')
    except zipfile.BadZipFile:
        raise ReadError(2,'input: invalid XLSX ZIP package') from None
